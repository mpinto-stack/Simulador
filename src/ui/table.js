...
export function buildTable(rows){ const thead=q('theadRow'); const tbody=q('tbl').querySelector('tbody'); const mode=(q('resMode').value||'net'); const view=(q('tblView').value||'ess'); const cols=visibleColsFor(view, mode); thead.innerHTML=''; tbody.innerHTML=''; cols.forEach(c=>{ const th=document.createElement('th'); th.textContent=headLabel(c); if(c==='marca') th.className='l sticky1'; if(c==='modelo') th.className='l sticky2'; thead.appendChild(th); });
  ...
  rows.forEach(r=>{ const tr=document.createElement('tr'); cols.forEach(c=>{ const td=document.createElement('td'); const val=r[c]; let text=''; let style=''; if(typeof val==='number'){ const isPct=(c==='pc_capex'||c==='pc_op'); const dec=isPct?1:((c.indexOf('tco_km')>=0||c.indexOf('eurCapex')>=0||c==='cons')?3:(c.indexOf('eur100')>=0?2:(c==='t1080'?1:(c.indexOf('tco_mes')>=0||c.indexOf('preco')>=0)?0:0))); text=isPct? (fmt(val,1)+'%') : fmt(val,dec); if(c==='eur100_n') style=heatStyle(val, hm.e100.min, hm.e100.max, true); if(c==='eurCapex_n') style=heatStyle(val, hm.cx.min, hm.cx.max, true); if(c==='tco_km_n') style=heatStyle(val, hm.tco.min, hm.tco.max, true); if(c==='t1080') style=heatStyle(val, hm.t.min, hm.t.max, true); }
